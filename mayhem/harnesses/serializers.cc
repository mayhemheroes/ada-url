// ADDITIVE OVERRIDE of upstream fuzz/serializers.cc (compiled by mayhem/build.sh in its place).
// The upstream harness aborts on valid inputs via two FALSE invariants -> 0 edges + 2 spurious
// defects in Mayhem. Removed here: (1) percent_encode then percent_decode treated as an identity
// (percent_decode collapses pre-existing %XX already present in the source, so it is not), and
// (2) is_normalized_windows_drive_letter treated as a subset of is_windows_drive_letter (they are
// independent predicates: e.g. a 3-char 'X:y' is normalized-true / windows-false). The same
// serializer/encoder/checker code paths are still exercised; the genuine parser-idempotency aborts
// (IPv4/IPv6 re-parse round-trips) are kept. Upstream fuzz/serializers.cc is left untouched.
...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* data, size_t size) {
  ...
  // ===== Percent Encode / Decode =====
  {
    ...
    // Exercise encode-then-decode on the PATH set. NOTE: percent_encode +
    // percent_decode is NOT an identity in general -- percent_decode collapses
    // ANY %XX sequence, including ones already present verbatim in the source
    // (the PATH set does not escape a literal '%'), so a source carrying
    // pre-existing percent-sequences round-trips to a different string. We only
    // drive both code paths here (no equality assertion); results are kept live
    // so the optimiser cannot elide the calls.
    {
  ...
  // every edge case is reachable without a valid URL structure.
  {
    ...
    // NOTE: is_normalized_windows_drive_letter and is_windows_drive_letter are
    // INDEPENDENT predicates, not a subset relationship. The normalized form
    // requires an exact two-char drive prefix; the windows form additionally
    // requires that prefix to be followed by a path separator or end-of-input,
    // so a 3-char input like a-colon-letter is normalized-true / windows-false.
    // A normalized letter does NOT imply a windows drive letter; we only
    // confirm both predicate calls run without crashing.
